...
gtp_modules_traceframe_info_get(void)
{
	...
	list_for_each_entry_rcu(mod, &(THIS_MODULE->list), list) {
		if (__module_address((unsigned long)mod)) {
            ...
            unsigned long core_text_size = mod->core_layout.text_size;
            //unsigned long core_size = mod->core_layout.text_size;
#else
            unsigned long module_core = (unsigned long)mod->module_core;
            unsigned long core_text_size = mod->core_text_size;
