...
谷歌工程师 Rahman Lavaee 宣布了他们利用 Propeller 实现了一套软件, 可以将代码预取插入到二进制文件, 可以减少 CPU 前端停顿, 以获得更快的性能, 特别是对于具有新预取指令的最新 Intel Granite Rapids 和 AMD Turin 处理器. [phoronix, 2025/10/22, Google Develops Code Prefetch Insertion Optimizer For Faster Intel GNR & AMD Turin Performance](https://www.phoronix.com/news/Propeller-Prefetch-Insertion)
...
LLVM 编译器工具链开始支持分布式 ThinLTO DTLTO, 作为处理 ThinLTO 编译以利用链路时优化的新方法. ThinLTO 是 LLVM 处理链路时优化的更具可扩展性和增量性的方法. 使用分布式 ThinLTO, 后端 ThinLTO 编译的分发可以通过外部分发系统完成. DTLTO 支持通过外部分发系统(例如 Incredibuild)分发后端 ThinLTO 编译. 对分发 ThinLTO 编译的现有支持通常涉及单独的精简链接(--thinlto-index-only)、后端编译以及由现代构建系统(如 Bazel)协调的链接步骤. 这种 Bazel 风格的分布式 ThinLTO 需要现代构建系统, 因为它必须处理摘要索引文件分片中指定的动态依赖项. 参见 [phoronix, 2025/07/21, LLVM Begins Landing Distributed ThinLTO "DTLTO" Support](https://www.phoronix.com/news/LLVM-DTLTO-Distributed-Thin).
...
| 2025/10/13 | Lyude Paul <lyude@redhat.com> | [Refcounted interrupts, SpinLockIrq for rust](https://lore.kernel.org/all/20251013155205.2004838-1-lyude@redhat.com) | 旨在为 Linux 内核引入支持 refcounted 中断和带中断禁用的 Rust 自旋锁(SpinLockIrq). 主要内容包括:<br>1.**核心改进**: <br>- 引入 per-CPU 计数器以跟踪 NMI 嵌套, 避免占用 refcounted IRQ 的位数; <br>- 提供 Rust 接口以控制本地中断, 增强 Rust 在内核同步机制中的支持;<br>- 实现 refcounted 中断机制, 替代原有非引用计数的中断 API;<br>- 提供 kunit 测试验证 refcounted 中断功能.<br>2. **转换计划与挑战**: <br>- 尝试将内核核心部分从旧 API 迁移至新 API, 初步实现系统引导;<br>- 发现旧 API 存在" 不平衡" 或" 顺序错误" 使用, 尤其在涉及底层指令( 如 x86 的 iret)时; <br>- 在 aarch64 平台发现 raw_spin_rq_unlock_irq 问题, 需修改上下文切换代码.<br>3.**后续计划**:<br>- 保留三个中断接口并存, 新驱动使用新API, 逐步完成迁移.<br>补丁共 17 项, 涉及多个架构和 Rust 模块, 代码改动较大, 已提交至 rust-next 分支. 参见 [LWN, 2025/10/15, A new API for interrupt-aware spinlocks](https://lwn.net/Articles/1039374/) | v13 ☐☑✓ | [2025/10/13, LORE v13, 0/17](https://lore.kernel.org/all/20251013155205.2004838-1-lyude@redhat.com) |
| 2025/07/12 | Mitchell Levy <levymitchell0@gmail.com> | [rust: Add Per-CPU Variable API](https://lore.kernel.org/all/20250712-rust-percpu-v2-0-826f2567521b@gmail.com) | 旨在为 Rust 添加 Per-CPU 变量的 API 支持, 便于在 Rust 中声明和使用 Per-CPU 变量, 并兼容 C 实现的 Per-CPU 变量访问( 需满足安全性要求). 新增了测试模块 `lib/percpu_test_rust. rs`, 并对文档和内存分配策略进行了改进. 主要集中在 `rust/kernel/percpu. rs` 等核心模块.  | v2 ☐☑✓ | [2025/07/12, LORE v2, 0/5](https://lore.kernel.org/all/20250712-rust-percpu-v2-0-826f2567521b@gmail.com) |
...
| 时间 | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:---:|:----:|:---:|:----:|:---------:|:----:|
| 2025/10/29 | Nathan Chancellor <nathan@kernel.org> | [fs/pipe: stop duplicating union pipe_index declaration](https://git.kernel.org/pub/scm/linux/kernel/git/kbuild/linux.git/commit/?id=5ff8ad3909524ad55297a434f87c238224825bf4) | Kbuild 启用 `-fms-extensions` 编译选项以及 jfs 头文件中重命名 `_inline` 以避免与 Clang 的 `-fms-extensions` 冲突. Christian Brauner 表示接受任何合入方式. [The Linux Kernel Looks To "Bite The Bullet" In Enabling Microsoft C Extensions](https://www.phoronix.com/news/Linux-6.19-Patch-Would-MS-Ext). | v1 ☐☑✓ 6.19-rc1 | [LORE](https://lore.kernel.org/all/20251029233057.GA3441561@ax162) |
...
| 时间 | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:---:|:----:|:---:|:----:|:---------:|:----:|
| 2022/09/27 | Oracle | [bpftune For BPF-Based](https://lore.kernel.org/all/20220927131518.30000-1-ojeda@kernel.org) | [Oracle Developing"bpftune"For BPF-Based, Automatic Tuning Of Linux Systems](https://www.phoronix.com/news/Oracle-bpftune)<br>*-*-*-*-*-*-*-* <br>[https://blogs.oracle.com/linux/post/introducing-bpftune](https://blogs.oracle.com/linux/post/introducing-bpftune)<br>*-*-*-*-*-*-*-* <br>[bpftune - Using Reinforcement Learning in BPF](https://blogs.oracle.com/linux/post/bpftune-using-reinforcement-learning-in-bpf)<br>*-*-*-*-*-*-*-* <br>[phoronix, 2025/12/13, Oracles Releases Updated "bpftune" For BPF-Based Auto-Tuning Of Linux Systems](https://www.phoronix.com/news/Oracle-bpftune-0.4-1) | v10 ☐☑✓ | [GitHub](https://github.com/oracle/bpftune) |
