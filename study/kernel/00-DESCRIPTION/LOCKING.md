...
| 2025/04/07 | Sebastian Andrzej Siewior <bigeasy@linutronix.de> | [futex: Add support task local hash maps, FUTEX2_NUMA and FUTEX2_MPOL](https://lore.kernel.org/all/20250407155742.968816-1-bigeasy@linutronix.de) | 这个补丁集旨在为 futex 引入任务本地哈希表支持, 并加入 Peter Zijlstra 提出的 FUTEX2_NUMA 和 FUTEX2_MPOL 特性. 主要改进包括: 重构本地哈希逻辑、基于 `vmalloc_huge_node()` 实现 `vmalloc_huge()`、优化唤醒机制、调整哈希桶调整大小逻辑、新增 `PR_FUTEX_HASH_SET_SLOTS` 接口支持 perf 基准测试等. 补丁还涉及多个 futex 核心逻辑优化、锁定机制调整及 ABI 头文件同步. 目标是提升在 NUMA 和内存策略多变环境下的 futex 性能和可扩展性. 参见 [phoronix, 2025/05/26, FUTEX2 Improvements Merged For Linux 6.16](https://www.phoronix.com/news/FUTEX2-Improvements-Linux-6.16). | v11 ☐☑✓ |[2025/04/07, LORE v11, 0/19](https://lore.kernel.org/all/20250407155742.968816-1-bigeasy@linutronix.de) |
| 2025/07/01 | Sebastian Andrzej Siewior | [futex: Temporary disable FUTEX_PRIVATE_HASH](https://lore.kernel.org/all/20250630145034.8JnINEaS@linutronix.de) | 临时禁用 `FUTEX_PRIVATE_HASH` 选项. 原因是 Chris Mason 报告了在大型系统上的显著性能退化 [2025/06/03, futex performance regression from "futex: Allow automatic allocation of process wide futex hash"](https://lore.kernel.org/all/3ad05298-351e-4d61-9972-ca45a0a50e33@meta.com/), 此问题源于每次 futex 操作调用 `rcuref_get() ` 引发的缓存行抖动. 补丁通过标记 `FUTEX_PRIVATE_HASH` 为 `BROKEN` 来禁用该功能, 以争取一个开发周期的时间修复性能问题. 后续版本将重新启用该选项. | v1 ☐☑✓ | [LORE](https://lore.kernel.org/all/20250630145034.8JnINEaS@linutronix.de) |
