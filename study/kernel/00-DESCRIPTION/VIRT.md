...
| 2025/11/10 | Wanpeng Li <kernellwp@gmail.com> | [sched/kvm: Semantics-aware vCPU scheduling for oversubscribed KVM](https://lore.kernel.org/all/20251110033232.12538-1-kernellwp@gmail.com) | 一项针对 KVM 超线程场景中 vCPU 调度优化的补丁系列, 旨在解决虚拟化环境中 yield_to() 机制效率低下的问题, 主要改进包括:<br>1. 调度器侧改进(vCPU Debooster):<br>在 `yield_to_task_fair()` 中引入有界 vruntime 惩罚机制, 提供比原有 buddy 机制更持久的调度偏好. 通过 cgroup 层级感知、EEVDF 调度器适配、惩罚自适应调节、反跳动机制和频率限制等手段, 减少锁持有者被抢占, 缓解"乒乓" 效应.<br>2. KVM 侧改进(IPI 感知的定向 yield): 在 `kvm_vcpu_on_spin()` 中加入 IPI 追踪机制, 识别发送与接收 IPI 的 vCPU 关系, 优先调度 IPI 接收者, 提升响应效率. 采用无锁访问、时间戳老化、EOI 清除机制和两轮候选选择策略, 确保高效与兼容.<br>该方案无需修改客户机操作系统, 适用于各类系统, 且具备运行时开关控制. 性能测试显示在适度超线程场景下, dbench 和 PARSEC 基准测试吞吐量提升显著, 尤其在 IPI 密集型任务中表现突出. 补丁按功能模块分片提交, 便于审查与调试. [phoronix, 2025/11/10, Tencent Proposes Semantics-Aware vCPU Scheduling For Over-Subscribed KVM Linux VMs](https://www.phoronix.com/news/Tencent-vCPU-Over-Scheduling). | v1 ☐☑✓ | [2025/11/10, LORE v1, 0/10](https://lore.kernel.org/all/20251110033232.12538-1-kernellwp@gmail.com) |
| 2026/01/21 | Stanislav Kinsburskii <skinsburskii@linux.microsoft.com> | [Introduce Hyper-V integrated scheduler support](https://lore.kernel.org/all/176903475057.166619.9437539561789960983.stgit@skinsburskii-cloud-desktop.internal.cloudapp.net) | 引入 Hyper-V 集成调度器(Integrated Scheduler)支持的补丁. 当前 Microsoft Hypervisor 有根调度器和核心调度器两种机制, 但在引入 L1 虚拟主机( L1VH) 后, 核心调度器导致 L1VH 无法有效控制其子分区的 vCPU 调度, 影响 cgroups、CFS 等机制的调度效果. 为此, Hyper-V 新增" 集成调度器" , 使 L1VH 可在其分配的物理核心上调度自身及子分区的 vCPU, 模拟根调度器行为, 同时保持系统其余部分使用核心调度器. | v1 ☐☑✓ | [2026/01/21, LORE v1, 0/2](https://lore.kernel.org/all/176903475057.166619.9437539561789960983.stgit@skinsburskii-cloud-desktop.internal.cloudapp.net) |
| 2025/12/04 | Srikar Dronamraju <srikar@linux.ibm.com> | [Steal time based dynamic CPU resource management](https://lore.kernel.org/all/20251204175405.1511340-1-srikar@linux.ibm.com) | 提出了一种基于 steal time(虚拟化中被其他虚拟机占用的时间) 的动态 CPU 资源管理机制, 旨在优化 PowerVM Shared LPARs 环境下的调度效率. 通过监控 steal time 来判断系统是否过载或欠载, 并据此动态调整 CPU 容量和可用性, 引导调度器减少资源争用, 提升整体性能. <br><br> 实验数据显示, 在非竞争 (nonoise) 和竞争 (noise) 场景下, 使用该补丁集后使用的 CPU 核心数减少, 同时缓存缺失、上下文切换、指令执行等指标平均降低约 3 倍, 性能有所提升. 尽管在某些线程数下存在性能回归, 但整体资源利用更高效. <br><br> 补丁集共 17 个, 涉及调度器、PowerPC 架构及 pseries 平台的多处修改, 支持软下线 / 上线 CPU、动态调整拓扑和容量、调试接口等功能. 作者欢迎社区反馈, 并指出未来可扩展支持其他架构或其他提示机制.  | v1 ☐☑✓ | [2025/12/04, LORE v1, 00/17](https://lore.kernel.org/all/20251204175405.1511340-1-srikar@linux.ibm.com) |
