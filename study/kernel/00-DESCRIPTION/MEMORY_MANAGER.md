...
| 6.14 | 2025/01/24 | [phoronix, 2025/01/24, Uncached Buffered I/O & Some Other Nice Memory Management Optimizations With Linux 6.14](https://www.phoronix.com/news/Linux-6.14-MM) |
...
| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2024/05/31 | Byungchul Park <byungchul@sk.com> | [LUF(Lazy Unmap Flush) reducing tlb numbers over 90%](https://lore.kernel.org/all/20240531092001.30428-1-byungchul@sk.com) | 这组补丁的主要目的是实现一种名为 LUF(Lazy Unmap Flush)的机制, 以大幅减少 TLB(Translation Lookaside Buffer)刷新的数量, 特别是在处理内存迁移时.  实现 LUF 机制, 延迟 TLB 刷新直到实际需要, 从而减少 TLB 射杀的次数; 优化内存迁移过程中的性能, 特别是在处理大量内存页时; 为 x86、ARM64 和 RISC-V 架构提供支持; 添加自测试用例, 确保 LUF 机制的正确性和有效性; 通过这些改动, 内存管理和性能得到了显著提升, 特别是在处理内存迁移和 TLB shootdown 时. 参见 [phoronix, 2025/02/20, Linux Lazy Unmap Flush "LUF" Reducing TLB Shootdowns By 97%, Faster AI LLM Performance](https://www.phoronix.com/news/Linux-Lazy-Unmap-Flush) | v11 ☐☑✓ | [LORE v11,0/12](https://lore.kernel.org/all/20240531092001.30428-1-byungchul@sk.com)<br>*-*-*-*-*-*-*-* <br>[2025/02/20, LORE v12,0/26](https://lore.kernel.org/all/20250220052027.58847-1-byungchul@sk.com) |
...
| 2023/04/10 | Huang, Ying <ying.huang@intel.com> | [mm,unmap: avoid flushing TLB in batch if PTE is inaccessible](https://patchwork.kernel.org/project/linux-mm/patch/20230410075224.827740-1-ying.huang@intel.com/) | 738356 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230410075224.827740-1-ying.huang@intel.com)<br>*-*-*-*-*-*-*-* <br>[LORE v1,0/1](https://lore.kernel.org/r/20230424065408.188498-1-ying.huang@intel.com) |
| 2023/08/04 | Byungchul Park <byungchul@sk.com> | [Reduce TLB flushes under some specific conditions](https://lore.kernel.org/all/20230804061850.21498-1-byungchul@sk.com) | 通过在迁移时保留源和目标作品集来延迟 TLB 刷新. 参见 phoronix 报道 [New Linux Optimization Patches Reduced TLB Flushes By Over 50% In Some Cases](https://www.phoronix.com/news/CXL-TLB-Flushes-50p-Optimize). | v1 ☐☑✓ | [LORE v1,0/2](https://lore.kernel.org/all/20230804061850.21498-1-byungchul@sk.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/6](https://lore.kernel.org/r/20230817080559.43200-1-byungchul@sk.com) |
...
| 2022/10/28 | Yicong Yang <yangyicong@huawei.com> | [arm64: support batched/deferred tlb shootdown during page reclamation/migration](https://patchwork.kernel.org/project/linux-mm/cover/20221028081255.19157-1-yangyicong@huawei.com/) | 虽然 ARM64 有硬件进行 tlb shootdown, 但是使用 tlbi 进行硬件广播的开销也不小. 一个最简单的微基准测试显示, 即使在只有 8 核的骁龙 888 上, 即使只分页一个进程映射的页面, ptep_clear_flush() 的开销 (perf top) 也达到了 5.36%. 当页面由多个进程映射或 HW 有更多 CPU 时, 由于 tlb shootdown 糟糕的可伸缩性, 成本应该会变得更高, 同样的基准测试在 100 核左右的 ARM64 服务器上可以导致 16.99% 的 CPU 消耗. 这个补丁集利用现有的 BATCHED_UNMAP_TLB_FLUSH. 只在第一阶段 arch_tlbbatch_add_mm() 中发送 tlbi 指令. 在骁龙 888 上的测试表明, 补丁集消除了 ptep_clear_flush() 的开销. 在骁龙 888 上, 即使是单个进程映射的一个页面, 微基准测试也要快 5%. 有了这个支持, 我们可以对内存回收和[迁移](https://lore.kernel.org/lkml/20220921060616.73086-1-ying.huang@intel.com) 做更多的优化. | v5 ☐☑ | [LORE 0/4](https://lore.kernel.org/lkml/20220707125242.425242-1-21cnbao@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/2](https://lore.kernel.org/r/20221028081255.19157-1-yangyicong@huawei.com)<br>*-*-*-*-*-*-*-* <br>[LORE v6,0/2](https://lore.kernel.org/r/20221115031425.44640-1-yangyicong@huawei.com)<br>*-*-*-*-*-*-*-* <br>[LORE v7,0/2](https://lore.kernel.org/r/20221117082648.47526-1-yangyicong@huawei.com)<br>*-*-*-*-*-*-*-* <br>[LORE v8,0/2](https://lore.kernel.org/r/20230329035512.57392-1-yangyicong@huawei.com)<br>*-*-*-*-*-*-*-* <br>[LORE v9,0/2](https://lore.kernel.org/r/20230410134352.4519-1-yangyicong@huawei.com)<br>*-*-*-*-*-*-*-* <br>[LORE v9,0/2](https://lore.kernel.org/r/20230518065934.12877-1-yangyicong@huawei.com)<br>*-*-*-*-*-*-*-* <br>[LORE v10,0/4](https://lore.kernel.org/r/20230710083914.18336-1-yangyicong@huawei.com)<br>*-*-*-*-*-*-*-* <br>[LORE v11,0/4](https://lore.kernel.org/r/20230717131004.12662-1-yangyicong@huawei.com) |


### 1.3.4 Optimize TLB flushes
-------


| 时间 | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:---:|:----:|:---:|:----:|:---------:|:----:|
| 2025/01/20 | Vinay Banakar <vny@google.com> | [mm: Optimize TLB flushes during page reclaim](https://lore.kernel.org/all/CALf+9Yc3WjbG89uRWiDC_qYshJ5z_9WCrbEJe42Vbv+gJjs26g@mail.gmail.com) | [phoronix, 2025/01/22, Very Promising Linux Patch Optimizes TLB Flushes During Page Reclamation](https://www.phoronix.com/news/Linux-Optimize-TLB-Flush-Reclaim) | v1 ☐☑✓ | [LORE](https://lore.kernel.org/all/CALf+9Yc3WjbG89uRWiDC_qYshJ5z_9WCrbEJe42Vbv+gJjs26g@mail.gmail.com) |
...
| 2023/06/26 | Ryan Roberts <ryan.roberts@arm.com> | [variable-order, large folios for anonymous memory](https://patchwork.kernel.org/project/linux-mm/cover/20230626171430.3167004-1-ryan.roberts@arm.com/) | [Large folios for anonymous memory](https://lwn.net/Articles/937239). | v1 ☐☑ | [LORE v1,0/10](https://lore.kernel.org/r/20230626171430.3167004-1-ryan.roberts@arm.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/5](https://lore.kernel.org/r/20230726095146.2826796-1-ryan.roberts@arm.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/5](https://lore.kernel.org/r/20230810142942.3169679-1-ryan.roberts@arm.com) |
| 2023/04/03 | Zi Yan <zi.yan@sent.com> | [Split a folio to any lower order folios](https://patchwork.kernel.org/project/linux-mm/cover/20230403201839.4097845-1-zi.yan@sent.com/) | 736544 | v3 ☐☑ | [LORE v3,0/7](https://lore.kernel.org/r/20230403201839.4097845-1-zi.yan@sent.com) |
| 2023/06/21 | Matthew Wilcox <willy@infradead.org> | [Remove pagevecs](https://patchwork.kernel.org/project/linux-mm/cover/20230621164557.3510324-1-willy@infradead.org) | 完成 pagevec 到 folio_batch 的转换. | v1 ☐☑ | [LORE v1,0/13](https://lore.kernel.org/r/20230621164557.3510324-1-willy@infradead.org) |
| 2023/07/15 | Matthew Wilcox (Oracle) <willy@infradead.org> | [Followup folio conversions for zswap](https://patchwork.kernel.org/project/linux-mm/cover/20230715042343.434588-1-willy@infradead.org/) | 766106 | v1 ☐☑ | [LORE v1,0/5](https://lore.kernel.org/r/20230715042343.434588-1-willy@infradead.org) |
...
| 2023/05/01 | Vishal Moola <vishal.moola@gmail.com> | [Split ptdesc from struct page](https://patchwork.kernel.org/project/linux-mm/cover/20230501192829.17086-1-vishal.moola@gmail.com/) | 参见 LWN 报道 [The proper time to split struct page](https://lwn.net/Articles/937839). | v2 ☐☑ | [LORE v2,0/34](https://lore.kernel.org/r/20230501192829.17086-1-vishal.moola@gmail.com)39268-1-willy@infradead.org/) | 709303 | v2 ☐☑ | [LORE v2,0/24](https://lore.kernel.org/r/20230105214631.3939268-1-willy@infradead.org)[LORE v4,0/34](https://lore.kernel.org/r/20230612210423.18611-1-vishal.moola@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/33](https://lore.kernel.org/r/20230622205745.79707-1-vishal.moola@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v7,0/31](https://lore.kernel.org/r/20230725042051.36691-1-vishal.moola@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v8,0/31](https://lore.kernel.org/r/20230731170332.69404-1-vishal.moola@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v9,0/31](https://lore.kernel.org/r/20230807230513.102486-1-vishal.moola@gmail.com) |
...
| 2025/01/24 | Anthony Yznaga <anthony.yznaga@oracle.com> | [Add support for shared PTEs across processes](https://lore.kernel.org/all/20250124235454.84587-1-anthony.yznaga@oracle.com) | [phoronix, 2025/01/26, Linux Patches Allow Sharing PTEs Between Processes - Can Mean Significant RAM Savings](https://www.phoronix.com/news/Linux-Sharing-PTEs-Processes) | v1 ☐☑✓ | [LORE v1,0/20](https://lore.kernel.org/all/20250124235454.84587-1-anthony.yznaga@oracle.com) |
...
| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2023/02/11 | Matthew Wilcox <willy@infradead.org> | [New page table range API](https://patchwork.kernel.org/project/linux-mm/cover/20230211033948.891959-1-willy@infradead.org/) | New arch interfaces for manipulating multiple pages | v1 ☐☑ | [LORE v1,0/7](https://lore.kernel.org/r/20230211033948.891959-1-willy@infradead.org)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/30](https://lore.kernel.org/r/20230227175741.71216-1-willy@infradead.org)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/34](https://lore.kernel.org/r/20230228213738.272178-1-willy@infradead.org)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/36](https://lore.kernel.org/r/20230315051444.3229621-1-willy@infradead.org)<br>*-*-*-*-*-*-*-* <br>[LORE v1,0/4](https://lore.kernel.org/r/20230530080731.1462122-1-fengwei.yin@intel.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/38](https://lore.kernel.org/r/20230710204339.3554919-1-willy@infradead.org)<br>*-*-*-*-*-*-*-* <br>[LORE v6,0/38](https://lore.kernel.org/r/20230802151406.3735276-1-willy@infradead.org) |
...
Gregory Price 在 LSFMMBPF 2024 介绍了 2024 年社区提出的 Weighted memory interleaving 方向的探索. 参见 LWN 报道 [2024/05/18, Extending the mempolicy interface for heterogeneous systems](https://lwn.net/Articles/973964)

| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2023/10/09 | Gregory Price <gourry.memverge@gmail.com> | [mm: mempolicy: Multi-tier weighted interleaving](https://lore.kernel.org/all/20231009204259.875232-1-gregory.price@memverge.com) | 参见 LWN 报道 [Weighted interleaving for memory tiering](https://lwn.net/Articles/948037). | v2 ☐☑✓ | [LORE v2,0/3](https://lore.kernel.org/all/20231009204259.875232-1-gregory.price@memverge.com) |
...
| 2024/02/20 | Gregory Price <gourry.memverge@gmail.com> | [mm/mempolicy: weighted interleave system default weights](https://lore.kernel.org/all/20240220202529.2365-1-gregory.price@memverge.com) | 参见 LWN 报道 [2024/05/18, Extending the mempolicy interface for heterogeneous systems](https://lwn.net/Articles/973964). | v1 ☐☑✓ | [LORE v1,0/1](https://lore.kernel.org/all/20240220202529.2365-1-gregory.price@memverge.com) |
...
| 2023/07/10 | Huang, Ying <ying.huang@intel.com> | [mm: PCP high auto-tuning](https://patchwork.kernel.org/project/linux-mm/cover/2023071006532.290366-1-ying.huang@intel.com/) | 不同工作负载的页面分配性能要求通常是不同的. 因此, 我们需要自动将每个 CPU 上的 PCP(Per-CPU Pageset)调高, 以优化页面分配性能. 在该补丁集中, 实现了基于分配/释放深度的 PCP 高自动调优算法. 该算法的基本思想是在足够短的时间内 (大约 1 秒) 检测重复分配和释放模式. 周期需要短, 以便快速响应分配和释放模式更改, 并控制不必要的缓存所浪费的内存.<br>1. 为了检测重复的分配和释放模式, 在每个 CPU 上计算每个调优周期 (1 秒) 的分配 / 释放深度.<br>2. 为了计算 alloc/free 深度, 跟踪 alloc 计数. 在从 PCP 分配页面时增加, 在向 PCP 释放页面时减少. 深度是后大值与前小值之间的最大分配计数差. 而自由深度是前一个大值与后一个小值之间的最大分配计数差.<br>3. 然后, 计算多个调谐周期的平均分配 / 自由深度, 旧的分配 / 自由深度在平均值中逐渐衰减.<br>4.  最后, 在默认值和最大 PCP 高之间夹紧后, 将 PCP 高设置为平均分配深度和平均自由深度的较小值.<br>5. 这样, 纯粹的分配或释放不会增加 PCP 值, 因为 PCP 没有帮助. | v1 ☐☑ | [LORE v1,0/2](https://lore.kernel.org/r/20230710065325.290366-1-ying.huang@intel.com) |
| 2023/09/26 | Huang Ying <ying.huang@intel.com> | [mm: PCP high auto-tuning](https://lore.kernel.org/all/20230926060911.266511-1-ying.huang@intel.com) | 不同工作负载的页面分配性能要求通常不同. 因此, 需要自动调整每个 CPU 上的 PCP(每 CPU 页面集)高电平, 以优化页面分配性能. 参见 [Intel Has Another Series Optimizing Linux Performance With PCP High Auto-Tuning](https://www.phoronix.com/news/Intel-Linux-PCP-High-Auto-Tune) | v1 ☐☑✓ | [LORE RFC,0/2](https://lore.kernel.org/all/20230710065325.290366-1-ying.huang@intel.com)<br>*-*-*-*-*-*-*-* <br>[LORE v1,0/10](https://lore.kernel.org/all/20230920061856.257597-1-ying.huang@intel.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/10](https://lore.kernel.org/all/20230926060911.266511-1-ying.huang@intel.com) |
...
在 6.4 版本中, 针对低内存系统的 SLOB 分配器被删除, 随后 SLAB 也被删除, 内核中只剩下 SLUB. 内核开发人员现在可以更自由地改进 SLUB, 而不用担心破坏其他 SLUB. LSFMMBPF 2024 上 SLAB 的 maintainer Vlastimil Babka 对 SLUB 机制的未来做了规划和展望, 参见 LWN 报道 [2024/05/20, What's next for the SLUB allocator](https://lwn.net/Articles/974138).
...
| 2023/05/23 | Vlastimil Babka <vbabka@suse.cz> | [mm/slab: rename CONFIG_SLAB to CONFIG_SLAB_DEPRECATED](https://lore.kernel.org/all/20230523091139.21449-1-vbabka@suse.cz) | 正如 [[LSF/MM/BPF TOPIC] SLOB+SLAB allocators removal and future SLUB improvements](https://lore.kernel.org/all/4b9fc9c6-b48c-198f-5f80-811a44737e5f@suse.cz) 所讨论的那样, 在没有提出反对意见的情况下, 弃用 SLAB 分配器. 重命名用户可见选项, 以便 CONFIG_SLAB=y 的用户在 make-oldconfig 期间得到一个带有解释的新提示, 而 make-olddefconfig 只会切换到 SLUB. 在所有存在 CONFIG_SLAB=y 的 defconfigs 中, 删除该行, 使其也切换到 SLUB. 参见 LWN 报道 [A slab allocator (removal) update](https://lwn.net/Articles/932201) 以及 [Patch Posted For Formally Deprecating The SLAB Allocator](https://www.phoronix.com/news/Linux-SLAB-Deprecated-Patch) | v1 ☐☑✓ | [LORE](https://lore.kernel.org/all/20230523091139.21449-1-vbabka@suse.cz) |
...
| 2023/11/20 | Vlastimil Babka <vbabka@suse.cz> | [remove the SLAB allocator](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/log/?id=ecf9a253ce120082ce0a8aff806c4de4865cfcc5) | TODO | v2 ☐☑✓ v6.8-rc1 | [2023/11/13, LORE v1,0/20](https://lore.kernel.org/all/20231113191340.17482-22-vbabka@suse.cz)<br>*-*-*-*-*-*-*-* <br>[LORE v2,00/21](https://lore.kernel.org/all/20231120-slab-remove-slab-v2-0-9c9c70177183@suse.cz) |
...
| 2023/03/10 | Vlastimil Babka <vbabka@suse.cz> | [remove SLOB and allow kfree() with kmem_cache_alloc()](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/log/?id=ae65a5211d90e54ae604012ce9cf234c48780929) | 不再推荐 SLOB 作为简单的分配器. SLOB 分配程序的设计目的是提高空间效率, 但却不具有可伸缩性. 参见 phoronix 报道 [Linux 6.4 Looking To Drop The SLOB Memory Allocator](https://www.phoronix.com/news/Linux-6.4-Looks-To-Drop-SLOB) 和 LWN 报道 [SLOB nears the end of the road](https://lwn.net/Articles/918344). | v1 ☑✓ 6.4-rc1 | [LORE v1,0/7](https://lore.kernel.org/all/20230310103210.22372-1-vbabka@suse.cz)<br>*-*-*-*-*-*-*-*<br>[LORE v2,0/6](https://lore.kernel.org/all/20230317104307.29328-1-vbabka@suse.cz) |
...
| 2023/04/07 | zhaoyang.huang <zhaoyang.huang@unisoc.com> | [mm: introduce defer free for cma](https://patchwork.kernel.org/project/linux-mm/patch/1680864131-4675-1-git-send-email-zhaoyang.huang@unisoc.com/) | 737932 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/1680864131-4675-1-git-send-email-zhaoyang.huang@unisoc.com)<br>*-*-*-*-*-*-*-* <br>[LORE v1,0/1](https://lore.kernel.org/r/1681116395-18633-1-git-send-email-zhaoyang.huang@unisoc.com) |
...
hakavlad 提供了简单的 shell 脚本和 oneshot systemd 服务 [hakavlad/mg-lru-helper](https://github.com/hakavlad/mg-lru-helper) , 可以在引导时或运行中启用或禁用多代 LRU, 并获取 MGLRU 的当前状态. 并梳理了 MGLRU 历史各个版本的 PATCHSET 以及相关报道. 并对比测试了 MGLRU 以及 ClassicLRU 的性能差异 [cache-tests/mg-LRU-v3_vs_classic-LRU](https://github.com/hakavlad/cache-tests/tree/main/mg-LRU-v3_vs_classic-LRU).
...
| 2023/02/20 | Qi Zheng <zhengqi.arch@bytedance.com> | [make slab shrink lockless](https://patchwork.kernel.org/project/linux-mm/cover/20230220091637.64865-1-zhengqi.arch@bytedance.com/) | 723376 | v1 ☐☑ | [LORE v1,0/5](https://lore.kernel.org/r/20230220091637.64865-1-zhengqi.arch@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/7](https://lore.kernel.org/r/20230223132725.11685-1-zhengqi.arch@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/8](https://lore.kernel.org/r/20230226144655.79778-1-zhengqi.arch@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/8](https://lore.kernel.org/all/20230307065605.58209-1-zhengqi.arch@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/8](https://lore.kernel.org/r/20230313112819.38938-1-zhengqi.arch@bytedance.com) |
| 2023/06/22 | Qi Zheng <zhengqi.arch@bytedance.com> | [use refcount+RCU method to implement lockless slab shrink](https://patchwork.kernel.org/project/linux-mm/cover/20230622085335.77010-1-zhengqi.arch@bytedance.com/) | [Shrinking shrinker locking overhead](https://lwn.net/Articles/944199). | v1 ☐☑ | [LORE v1,0/29](https://lore.kernel.org/r/20230622085335.77010-1-zhengqi.arch@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/47](https://lore.kernel.org/r/20230724094354.90817-1-zhengqi.arch@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/49](https://lore.kernel.org/r/20230727080502.77895-1-zhengqi.arch@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/48](https://lore.kernel.org/r/20230807110936.21819-1-zhengqi.arch@bytedance.com) |
| 2023/08/16 | Qi Zheng <zhengqi.arch@bytedance.com> | [use refcount+RCU method to implement lockless slab shrink (part 1)](https://patchwork.kernel.org/project/linux-mm/cover/20230816083419.41088-1-zhengqi.arch@bytedance.com/) | 776546 | v1 ☐☑ | [LORE v1,0/5](https://lore.kernel.org/r/20230816083419.41088-1-zhengqi.arch@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/5](https://lore.kernel.org/r/20230817112402.77010-1-zhengqi.arch@bytedance.com) |
...
| 2023/02/02 | Yosry Ahmed <yosryahmed@google.com> | [Ignore non-LRU-based reclaim in memcg reclaim](https://patchwork.kernel.org/project/linux-mm/cover/20230202233229.3895713-1-yosryahmed@google.com/) | 在使用 memory.reclaim 运行一些主动回收测试时, 我们注意到一些测试出现了问题: 即使我们没有完全回收请求的数量, 写入 memory.reclain 也会成功. 分析发现存在过度报告 MEMCG 回收中回收的页面数量. 通过基于 LRU 的回收以外的其他方式回收的页面, 通过 task_struct->structscan_control 中的回收状态进行跟踪. 这些页面通过 LRU 添加到回收页面的数量中. 对于 memcg 回收, 这些页面通常无法链接到回收下的 MEMCG, 并且可能导致回收页面的数量被高估. 这组补丁试图解决这个问题.<br>补丁 1-2 只是重构, 它们添加了一些辅助程序, 这些辅助程序包装了当前回收状态上的一些操作, 并重命名 reclaim_state->reclaimed_slab 为 reclaim_state->reclaimed. 核心补丁 3 会忽略 MEMCG 回收中在 LRU 回收之外回收的页面. 无论如何, 这些页面都是 uncharged 的, 所以即使我们最终少报了回收的页面, 我们仍然会在 charging 过程中取得进展. | v1 ☐☑ | [LORE v1,0/2](https://lore.kernel.org/r/20230202233229.3895713-1-yosryahmed@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v1,0/2](https://lore.kernel.org/r/20230228085002.2592473-1-yosryahmed@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/3](https://lore.kernel.org/r/20230309093109.3039327-1-yosryahmed@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/3](https://lore.kernel.org/r/20230331070818.2792558-1-yosryahmed@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/3](https://lore.kernel.org/r/20230404001353.468224-1-yosryahmed@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/2](https://lore.kernel.org/r/20230405185427.1246289-1-yosryahmed@google.com) |
| 2022/11/10 | Lu Jialin <lujialin4@huawei.com> | [mm/memcontrol.c: drains percpu charge caches in memory.reclaim](https://patchwork.kernel.org/project/linux-mm/patch/20221110065316.67204-1-lujialin4@huawei.com) | 当用户使用 memory.reclaim 回收内存时, 在耗尽 percpu-lru 缓存后, 通过 drain_all_stock() 释放 percpu 对给定 memcg 库存的缓存, 以期引入更多可收回的页面. | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20221110065316.67204-1-lujialin4@huawei.com) |
...
PowerPC 体系结构 (POWER10) 支持热/冷页面跟踪功能(Hot/Cold page tracking), 参见 [HotChips2020-Server_Processors_IBM_Starke_POWER10_v33](https://hc32.hotchips.org/assets/program/conference/day1/HotChips2020_Server_Processors_IBM_Starke_POWER10_v33.pdf), 该功能以可配置的页面大小粒度, 提供访问计数器和访问关联详细信息. 早期 RFC 版本可以在 [kvaneesh/linux](https://github.com/kvaneesh/linux/commit/b472e2c8080823bb4114c286270aea3e18ffe221) 找到. 参见 LWN 报道 [Page aging with hardware counters](https://lwn.net/Articles/931812).
...
内核不提供在内存压力下保护工作集的方法. 用户空间需要一定数量的匿名和干净的文件页才能正常运行. 首先, 用户空间需要共享库和可执行二进制文件的缓存. 如果干净文件页的数量低于某个级别, 则可能会发生抖动甚至死锁.

[`LE9-PATCH`](https://github.com/hakavlad/le9-patch) 提供了一些 sysctl 旋钮, 用于对干净的文件页面进行软保护和硬保护, 从而在内存压力下保护 Working Set(匿名和干净的文件页).

| 时间 | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:---:|:----:|:---:|:----:|:---------:|:----:|
| 2010/10/28 | Mandeep Singh Baines <msb@chromium.org> | [RFC: vmscan: add min_filelist_kbytes sysctl for protecting the working set](https://lore.kernel.org/all/20101028191523.GA14972@google.com) | Linux 桌面在沉重的内存压力下可能会非常难以忍受, 正如多年来所展示的那样, 如今越来越多的注意力转向 OOMD/systemd-oomd 和其他替代方案, 以更好地处理 Linux 内存不足/内存不足的情况, 尤其是当今的桌面软件和 Web 浏览器消耗越来越多的内存. 帮助这种情况的另一项努力是 "le9" Linux 内核补丁. 保护干净的文件页(页面缓存)可用于防止抖动、减少内存压力下的 I/O、避免高延迟并防止在接近 OOM 的条件下发生活锁. 当前的 le9 补丁提供了两个 sysctl 旋钮, 用于对干净的文件页面进行软保护和硬保护. 当前的 le9 补丁基于最初由 Mandeep Singh Baines (2010) 和 Marcus Linsner (2018-2019) 创建的补丁. 参见 ["le9" Strives To Make Linux Very Usable On Systems With Small Amounts Of RAM](https://www.phoronix.com/news/le9-Linux-Low-RAM), [le9-patch, Add sysctl knobs for protecting the working set](https://github.com/hakavlad/le9-patch). | v1 ☐☑✓ | [LORE](https://lore.kernel.org/all/20101028191523.GA14972@google.com) |
...
| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2022/11/15 | Nhat Pham <nphamcs@gmail.com> | [cachestat: a new syscall for page cache state of files](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/log/?id=88537aac0b3840314d3a5cc401a7fc400c069bc9) | 内核中的 page cache 会将文件的 page 保存在 RAM 中, 让这些 page 可以不需要每次都要浪费从持久性存储设备上读取的时间就可以直接访问到. 应用程序通常完全不知道 page cache 的相关行为; 它加快了速度, 这是最重要的一点. 不过, 有些应用程序可以从了解某个文件某一时刻有多少在 page cache 中, 从而得到一些改善. Nhat Pham 提出的 cachestat() 系统调用就是一系列尝试中的最新成果, 从而可以将这些信息提供出来. 参见 LWN 报道 [Checking page-cache status with cachestat()](https://lwn.net/Articles/917096) 和 [Linux 6.5 Brings New "cachestat" Syscall For Querying Page Cache Stats Of A File](https://www.phoronix.com/news/Linux-6.5-MM-cachestat). | v1 ☐☑ 6.5-rc1 | [LORE v1,0/4](https://lore.kernel.org/r/20221115182901.2755368-1-nphamcs@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/4](https://lore.kernel.org/r/20221205175140.1543229-1-nphamcs@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/4](https://lore.kernel.org/r/20221208223104.1554368-1-nphamcs@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/4](https://lore.kernel.org/r/20221216192149.3902877-1-nphamcs@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/3](https://lore.kernel.org/r/20230104231127.2634648-1-nphamcs@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v7,0/3](https://lore.kernel.org/all/20230124021118.154078-1-nphamcs@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v8,0/3](https://lore.kernel.org/r/20230126175356.1582123-1-nphamcs@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v12,0/3](https://lore.kernel.org/r/20230421231421.2401346-1-nphamcs@gmail.com)<br>*-*-*-*-*-*-*-* <br>[LORE v13,0/3](https://lore.kernel.org/r/20230503013608.2431726-1-nphamcs@gmail.com) |
| 2023/05/10 | Nhat Pham <nphamcs@gmail.com> | [cachestat: wire up cachestat for other architectures](https://patchwork.kernel.org/project/linux-mm/patch/20230510195806.2902878-1-nphamcs@gmail.com/) | 746532 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230510195806.2902878-1-nphamcs@gmail.com) |
| 2023/05/11 | Nhat Pham <nphamcs@gmail.com> | [arm64: wire up cachestat for arm64](https://patchwork.kernel.org/project/linux-mm/patch/20230511092843.3896327-1-nphamcs@gmail.com/) | 746681 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230511092843.3896327-1-nphamcs@gmail.com) |
...
| 2023/05/17 | Jiaqi Yan <jiaqiyan@google.com> | [Improve hugetlbfs read on HWPOISON hugepages](https://patchwork.kernel.org/project/linux-mm/cover/20230517160948.811355-1-jiaqiyan@google.com/) | 748541 | v1 ☐☑ | [LORE v1,0/3](https://lore.kernel.org/r/20230517160948.811355-1-jiaqiyan@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/4](https://lore.kernel.org/r/20230707201904.953262-1-jiaqiyan@google.com) |
...
| 2022/08/02 | Joao Martins <joao.m.martins@oracle.com> | [[v1] mm/hugetlb_vmemmap: remap head page to newly allocated page](https://patchwork.kernel.org/project/linux-mm/patch/20220802180309.19340-1-joao.m.martins@oracle.com/) | 664896 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20220802180309.19340-1-joao.m.martins@oracle.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/1](https://lore.kernel.org/r/20221109200623.96867-1-joao.m.martins@oracle.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/1](https://lore.kernel.org/r/20221110121214.6297-1-joao.m.martins@oracle.com) |
| 2023/04/12 | Pasha Tatashin <pasha.tatashin@soleen.com> | [mm: hugetlb_vmemmap: provide stronger vmemmap allocaction gurantees](https://patchwork.kernel.org/project/linux-mm/patch/20230412152337.1203254-1-pasha.tatashin@soleen.com/) | 739189 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230412152337.1203254-1-pasha.tatashin@soleen.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/1](https://lore.kernel.org/r/20230412195939.1242462-1-pasha.tatashin@soleen.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/1](https://lore.kernel.org/r/20230508234059.2529638-1-pasha.tatashin@soleen.com) |
...
| 2022/05/27 | Jiaqi Yan <jiaqiyan@google.com> | [Memory poison recovery in khugepaged collapsing](https://patchwork.kernel.org/project/linux-mm/cover/20220527190731.322722-1-jiaqiyan@google.com/) | 645671 | v4 ☐☑ | [LORE v4,0/2](https://lore.kernel.org/r/20220527190731.322722-1-jiaqiyan@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v6,0/2](https://lore.kernel.org/r/20221107025359.2911028-1-jiaqiyan@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v7,0/2](https://lore.kernel.org/r/20221118013157.1333622-1-jiaqiyan@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v8,0/2](https://lore.kernel.org/r/20221201005931.3877608-1-jiaqiyan@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v9,0/2](https://lore.kernel.org/r/20221205234059.42971-1-jiaqiyan@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v11,0/3](https://lore.kernel.org/r/20230327211548.462509-1-jiaqiyan@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v12,0/3](https://lore.kernel.org/r/20230329151121.949896-1-jiaqiyan@google.com) |
...
| 时间 | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:---:|:----:|:---:|:----:|:---------:|:----:|
| 2013/11/12  | Bob Liu <lliubbo@gmail.com> | [mm: thp: khugepaged: add policy for finding target node](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=9f1b868a13ac36bd207a571f5ea1193d823ab18d) | Khugepaged 将扫描 / 释放 HPAGE_PMD_NR 正常页面, 并替换为从第一个扫描的正常页面的节点分配的 hugepage, 但此策略过于粗糙, 可能会破坏所有节点之间的原始页面平衡.<br>1. 问题 1, 如果第一个扫描的正常页面是从节点 A 分配的, 那么大多数其他扫描的正常页都是从节点 B 或 C 分配的. 但是 hugepage 总是从节点 A 中分配 hugepage, 这会给节点 A 带来额外的内存压力. 通过让 khugepaged 从扫描正常页面的最大记录命中的节点分配 hugepage 来解决这个问题, 这样可以最大限度地减少对原始页面平衡的影响.<br>2. 问题 2, 如果从节点 A、B 和 C 平均分配正常扫描的页面, 则在 khugepaged 启动后, 节点 A 仍将承受额外的内存压力. 这组补丁通过从具有相同记录的所有节点分配 hugepage 循环来解决此问题. | v1 ☐☑ | [COMMIT](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=9f1b868a13ac36bd207a571f5ea1193d823ab18d) |
| 2022/11/03 | Yang Shi <shy828301@gmail.com> | [[v2,1/2] mm: khugepaged: allow page allocation fallback to eligible nodes](https://patchwork.kernel.org/project/linux-mm/patch/20221103213641.7296-1-shy828301@gmail.com/) | khugepaged 代码会选择命中率最高的节点作为首选节点, 如果几个节点有相同的命中记录, 也会尝试进行一些平衡. 但有一种极端情况, 与 MADV_COLLAPSE 类似, 即不存在的节点可能会被返回为首选节点. 因此, 使用 nodemask 来记录具有相同命中记录的节点, 这样大容量分配可以回退到这些节点. 此补丁删除了 `__GFP_THESNODE`, 因为如果 nodemask 为空(没有设置节点), 这意味着有一个节点具有最多的历史记录, 那么 nodemask 方法实际上的行为类似于 `__GFP_THESNODE`. | v2 ☐☑ | [LORE v2,0/2](https://lore.kernel.org/r/20221103213641.7296-1-shy828301@gmail.com) |
...
| 2023/07/28 | Muhammad Usama Anjum <usama.anjum@collabora.com> | [WIP: Performance improvements](https://patchwork.kernel.org/project/linux-mm/patch/6b6a4e1c-a9e9-9592-d5b4-3c9210c8b650@collabora.com/) | 770530 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/6b6a4e1c-a9e9-9592-d5b4-3c9210c8b650@collabora.com) |
...
| 2023/01/09 | Suren Baghdasaryan <surenb@google.com> | [Per-VMA locks](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/log/?id=c7f8f31c00d187a2c71a241c7f2bd6aa102a4e6f) | 开启了 x86,ARM64,POWER 架构. | v1 ☐☑ 6.4-rc1 | [LORE v1,0/41](https://lore.kernel.org/r/20230109205336.3665937-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/33](https://lore.kernel.org/r/20230127194110.533103-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,00/35](https://lore.kernel.org/all/20230216051750.3125598-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/33](https://lore.kernel.org/all/20230227173632.3292573-1-surenb@google.com) |
| 2023/03/14 | Heiko Carstens <hca@linux.ibm.com> | [s390/mm: try VMA lock-based page fault handling first](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/log/?id=e06f47a16573decc57498f2d02f9af3bb3e84cf2) | 729931 | v1 ☐☑ 6.4-rc1 | [LORE v1,0/1](https://lore.kernel.org/r/20230314132808.1266335-1-hca@linux.ibm.com) |
| 2023/04/04 | Matthew Wilcox (Oracle) <willy@infradead.org> | [Avoid the mmap lock for fault-around](https://lore.kernel.org/all/20230404135850.3673404-1-willy@infradead.org) | linux-next 树目前包含一些补丁 (主要来自 Suren), 这些补丁可以在没有 mmap 锁保护的情况下处理一些页面错误. 这个补丁集增加了在不使用 mmap 锁的情况下处理页缓存中已经存在的部分文件的页错误的能力. 作者采取了一种非常渐进的方法来解锁. | v1 ☐☑✓ | [LORE v1,0/6](https://lore.kernel.org/all/20230404135850.3673404-1-willy@infradead.org)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/9](https://lore.kernel.org/r/20230711202047.3818697-1-willy@infradead.org) |
| 2023/04/14 | Suren Baghdasaryan <surenb@google.com> | [mm: handle swap page faults if the faulting page can be locked](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=ea65d9a7db3819b26e673ea987eb830ed207afd0) | 当在 VMA 锁定保护下处理页面故障时, 所有交换页面故障(swap page faults)都会使用 mmap_lock 重试, 因为如果不能立即锁定 folio, 则 folio_lock_or_retry 实现必须放弃并重新获取 mmap_lock. 这个补丁修改为只在对开锁定失败时重试, 而不是重试所有交换的页面错误. | v1 ☐☑✓ | [LORE v1,0/1](https://lore.kernel.org/all/20230414180043.1839745-1-surenb@google.com) |
| 2023/04/28 | Suren Baghdasaryan <surenb@google.com> | [mm: change per-VMA lock statistics to be disabled by default](https://patchwork.kernel.org/project/linux-mm/patch/20230428173533.18158-1-surenb@google.com) | 743905 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230428173533.18158-1-surenb@google.com) |
| 2023/06/09 | Suren Baghdasaryan <surenb@google.com> | [Per-vma lock support for swap and userfaults](https://patchwork.kernel.org/project/linux-mm/cover/20230609005158.2421285-1-surenb@google.com/) | 755484 | v2 ☐☑ | [LORE v2,0/6](https://lore.kernel.org/r/20230609005158.2421285-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/8](https://lore.kernel.org/r/20230627042321.1763765-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/6](https://lore.kernel.org/r/20230628071800.544800-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/6](https://lore.kernel.org/r/20230628172529.744839-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v6,0/6](https://lore.kernel.org/r/20230630020436.1066016-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v7,0/6](https://lore.kernel.org/r/20230630211957.1341547-1-surenb@google.com) |
| 2023/07/04 | Suren Baghdasaryan <surenb@google.com> | [[1/1] fork: lock VMAs of the parent process when forking](https://patchwork.kernel.org/project/linux-mm/patch/20230704200656.2526715-1-surenb@google.com/) | 762467 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230704200656.2526715-1-surenb@google.com) |
| 2023/07/05 | Suren Baghdasaryan <surenb@google.com> | [Avoid memory corruption caused by per-VMA locks](https://patchwork.kernel.org/project/linux-mm/cover/20230705063711.2670599-1-surenb@google.com/) | 762531 | v2 ☐☑ | [LORE v2,0/2](https://lore.kernel.org/r/20230705063711.2670599-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/2](https://lore.kernel.org/r/20230705171213.2843068-1-surenb@google.com)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/2](https://lore.kernel.org/r/20230706011400.2949242-1-surenb@google.com) |
| 2023/07/24 | Matthew Wilcox (Oracle) <willy@infradead.org> | [Handle most file-backed faults under the VMA lock](https://patchwork.kernel.org/project/linux-mm/cover/20230724185410.1124082-1-willy@infradead.org/) | 769002 | v3 ☐☑ | [LORE v3,0/10](https://lore.kernel.org/r/20230724185410.1124082-1-willy@infradead.org) |
| 2023/07/31 | Suren Baghdasaryan <surenb@google.com> | [make vma locking more obvious](https://patchwork.kernel.org/project/linux-mm/cover/20230731171233.1098105-1-surenb@google.com/) | 771308 | v1 ☐☑ | [LORE v1,0/6](https://lore.kernel.org/r/20230731171233.1098105-1-surenb@google.com) |
| 2023/08/21 | Kefeng Wang <wangkefeng.wang@huawei.com> | [mm: convert to generic VMA lock-based page fault](https://patchwork.kernel.org/project/linux-mm/cover/20230821123056.2109942-1-wangkefeng.wang@huawei.com/) | 777870 | v2 ☐☑ | [LORE v2,0/10](https://lore.kernel.org/r/20230821123056.2109942-1-wangkefeng.wang@huawei.com) |
...
| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2023/05/22 | Uladzislau Rezki (Sony) <urezki@gmail.com> | [Mitigate a vmap lock contention](https://lore.kernel.org/all/20230522110849.2921-1-urezki@gmail.com) | 缓解 vmap lock(free_vmap_area_lock) 的争用. | v1 ☐☑✓ | [LORE v1,0/9](https://lore.kernel.org/all/20230522110849.2921-1-urezki@gmail.com) |



### 8.2.6 DEVMEM Page-Fault
-------

| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2023/03/19 | buddy.zhang <buddy.zhang@biscuitos.cn> | [mm: Keep memory type same on DEVMEM Page-Fault](https://patchwork.kernel.org/project/linux-mm/patch/20230319033750.475200-1-buddy.zhang@biscuitos.cn/) | 在 X86 架构上, 支持 Page 表上的内存类型, 例如 PTE 是 PAT/PCD/PWD, 可以将内存类型设置为 WC/WB/WT/UC 等.<br>然后, 来自用户空间或内核空间的虚拟地址可以映射到相同的物理页面, 如果每个页面表都有不同的内存类型, 那么同一物理页面有更多的内存类型是令人困惑的.<br>在 DEVMEM 上, remap_pfn_range() 在不同的映射上保持内存类型相同. | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230319033750.475200-1-buddy.zhang@biscuitos.cn) |
...
| 2023/05/06 | 程垲涛 Chengkaitao Cheng <chengkaitao@didiglobal.com> | [memcontrol: support cgroup level OOM protection](https://patchwork.kernel.org/project/linux-mm/cover/20230506114948.6862-1-chengkaitao@didiglobal.com) | 建立了一种新的 OOM 评分算法, 支持 C 组级 OOM 保护机制. 当全局 / memcg-oom 事件发生时, 我们将 cgroup 中的所有进程视为一个整体, oom 杀手需要根据 cgroup 的保护配额选择要杀死的进程. | v3 ☐☑ | [LORE v3,0/2](https://lore.kernel.org/r/20230506114948.6862-1-chengkaitao@didiglobal.com) |
...
| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2023/04/10 | Gang Li <ligang.bdlg@bytedance.com> | [mm: oom: introduce cpuset oom](https://patchwork.kernel.org/project/linux-mm/patch/20220921064710.89663-1-ligang.bdlg@bytedance.com/) | 678906 | v1 ☐☑ | [2022/09/21 LORE v1,0/1](https://lore.kernel.org/r/20220921064710.89663-1-ligang.bdlg@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[2023/04/10 LORE v3,0/1](https://lore.kernel.org/r/20230410025056.22103-1-ligang.bdlg@bytedance.com)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/1](https://lore.kernel.org/r/20230411134539.45046-1-ligang.bdlg@bytedance.com) |

*   Other

| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2023/04/26 | Yosry Ahmed <yosryahmed@google.com> | [memcg: OOM log improvements](https://patchwork.kernel.org/project/linux-mm/cover/20230426133919.1342942-1-yosryahmed@google.com/) | 743348 | v1 ☐☑ | [LORE v1,0/2](https://lore.kernel.org/r/20230426133919.1342942-1-yosryahmed@google.com) |
...
[[LSF/MM/BPF TOPIC] Reducing zombie memcgs](https://lore.kernel.org/all/CABdmKX2M6koq4Q0Cmp_-=wbP0Qa190HdEGGaHfxNS05gAkUtPA@mail.gmail.com)
...
[CXL 实践指南：关于内存扩展，需要知道的是...](https://cloud.tencent.com/developer/article/2517444)
...
[Utilizing Linux NUMA in CMM-D: Memory Tiering, 2024/05/07](https://semiconductor.samsung.com/news-events/tech-blog/utilizing-linux-numa-in-cmm-d-memory-tiering/)
...
| 2022/07/13 | Huang Ying <ying.huang@intel.com> | [memory tiering: hot page selection](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/log/?id=c959924b0dc53bf6252793f41480bc01b9792570) | 在分级内存的系统中, 需要使用 NUMA 来优化页面在内存中的位置. 需要识别慢内存节点中的热门页面.<br>最初的 NUMA Balancing 实现选择并提升最近访问最多(mostly recently accessed/MRU) 的页面. 但这并不是一个识别热门页面的完美算法, 访问频率很低的页面最终可能会被访问, 而最近访问的页面也可能很冷. 因为 NUMA 平衡页表扫描周期可能很长 (例如 60 秒). 因此, 这个补丁集<br>1. 基于 NUMA 平衡页表扫描和页面 Page Fault & Hint 之间的延迟实现了一个新的热页识别算法, 这是一种最常访问(MFU) 算法.<br>1. 另外一方面热页升级可能会在系统中产生一些开销. 在内存分层模式下的 NUMA 均衡, 如果慢速内存节点有热页, 快速内存节点有冷页, 则需要在快、冷内存节点之间提升热页/降级冷页. 由于访问膨胀和缓慢的内存带宽争用, 提升/降级所消耗的 CPU 周期和内存带宽将损害某些工作负载的延迟. 因此有必要尽快地完成提升/降级. 为了控制开销, 实现了一种页面提升率限制机制.<br>3. 用于识别热页的升级热阈值通常取决于工作负载. 因此这个补丁还实现了一个热页阈值自动调整算法. 基本思想是增加/减少热页阈值, 使通过热页阈值提升到快速内存的页面数接近速率限制. | v1 ☐☑ | [2022/04/08 LORE v1,0/3](https://lore.kernel.org/r/20220408071222.219689-1-ying.huang@intel.com)<br>*-*-*-*-*-*-*-* <br>[2022/04/26 LORE v2,0/3](https://lore.kernel.org/r/20220426085105.60822-1-ying.huang@intel.com)<br>*-*-*-*-*-*-*-* <br>[2022/05/01 LORE v2,0/3,RESEND](https://lore.kernel.org/r/20220510063958.86985-1-ying.huang@intel.com)<br>*-*-*-*-*-*-*-* <br>[2022/06/22 LORE v4,0/3](https://lore.kernel.org/r/20220622083519.708236-1-ying.huang@intel.com)<br>*-*-*-*-*-*-*-* <br>[2022/07/13 LORE v4,0/3,RESEND](https://lore.kernel.org/r/20220713083954.34196-1-ying.huang@intel.com) |
| 2022/04/13 | Jagdish Gediya <jvgediya@linux.ibm.com> | [mm: demotion: Introduce new node state N_DEMOTION_TARGETS](https://patchwork.kernel.org/project/linux-mm/cover/20220413092206.73974-1-jvgediya@linux.ibm.com/) | 这组补丁引入了新的节点状态 N_DEMATION_TARGETS, 用于区分可作为降级目标的节点, node_states[N_DEMATION_TARGETS] 用于保存可作为降级对象的节点列表. 节点状态 N_DEMOTION_TARGETS 也是从 dax kmem 驱动程序设置的, 通过 dax kmem 注册的特定类型的存储器(例如 HBM)可能不是降级的正确选择, 因此将来应该根据某些属性来区分它们, dax kmem - 驱动程序应该避免将它们设置为 N_DEMOTON_TARGES, 然而, 当前的实现也没有区分任何这样的内存, 它将所有 N_memory 都视为降级目标, 因此该补丁系列不会修改当前的行为.<br>可以通过 `cat /sys/devices/system/node/demotion` 可用于查看系统中可用的降级目标. | v2 ☐☑ | [LORE v2,0/5](https://lore.kernel.org/r/20220413092206.73974-1-jvgediya@linux.ibm.com)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/7](https://lore.kernel.org/r/20220422195516.10769-1-jvgediya@linux.ibm.com) |
...
| 2022/11/22 | Mina Almasry <almasrymina@google.com> | [mm: Add memory.demote for proactive demotion only](https://patchwork.kernel.org/project/linux-mm/patch/20221122203850.2765015-2-almasrymina@google.com/) | 添加主动降级接口 memory.demote.<br>此接口可按如下方式使用:<br>`echo "1m" > memory.demote`. 在这个命令下, 内核将尝试从这个 cgroup 中降级 1M 的内存. 内核可能无法降级用户空间请求的全部数量, 在这种情况下, EAGAIN 将返回给用户(类似于 memory.request). 内核将只尝试使用此接口降级页面. 它不会尝试任何其他类型的回收(交换、写回或回收干净的文件页). | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20221122203850.2765015-2-almasrymina@google.com) |
| 2023/08/16 | Huang, Ying <ying.huang@intel.com> | [memory tiering: calculate abstract distance based on ACPI HMAT](https://patchwork.kernel.org/project/linux-mm/cover/20230816080024.105554-1-ying.huang@intel.com/) | 我们有明确的内存层框架来管理具有多种类型内存的系统, 例如 DIMM 插槽中的 DRAM 和 CXL 内存设备. 其中, 相同类型的内存设备将被分组为内存类型, 然后放入内存层中. 为了描述内存类型的性能, 定义了抽象距离. 这与内存延迟成正比, 与内存带宽成反比. 为了使代码尽可能简单, dax/kmem 中使用固定的抽象距离来描述慢速内存, 例如 Optane DCPMM.<br>为了支持更多的内存类型, 在本系列中, 添加了抽象距离计算算法管理机制, 提供了基于 ACPI HMAT 的算法实现, 并在 dax/kmem 驱动程序中使用了通用的抽象距离计算接口. 因此, dax/kmem 除了原始的 Optane DCPMM 之外, 还可以支持 HBM(高带宽内存). | v1 ☐☑ | [LORE v1,0/4](https://lore.kernel.org/r/20230816080024.105554-1-ying.huang@intel.com) |


#### 12.1.2.3 硬件驱动的冷热页识别与页面分级
-------

[vTMM: Tiered Memory Management for Virtual Machines——论文泛读](https://zhuanlan.zhihu.com/p/688423783)

| 日期 | 概要 | 论文 / 链接 | 团队 | 描述 |
|:---:|:----:|----------:|:----:|:----:|
| 2021/07 | HeMem, 使用 PMU 跟踪各种内存访问的次数(可以区分读和写、DRAM 访问和 NVM 访问), 从而而区分冷热页, 最终实现将冷热数据分别存放在 NVM 和 DRAM | [HeMem: Scalable Tiered Memory Management for Big Data Applications and Real NVM, SOSP'21](https://zhuanlan.zhihu.com/p/461022129) | 团队 | 提出了 HeMem, 目标是将冷热数据分别存放在 NVM 和 DRAM 上, 其设计包括冷热数据的区分和数据迁移策略两个部分. 之前的软件管理方法通过访问位/脏数据位来判断一个内存页是否被访问过. 这种机制需要软件遍历页表项, 还会导致 TLB shootdown(重置 bit). HeMem 使用采样(Sampling)的方法来统计访存分布, 具有更低的开销和更好的扩展性. HeMem 使用 CPU 内部的性能计数器来跟踪各种内存访问的次数(可以区分读和写、DRAM 访问和 NVM 访问). HeMem 维护一个异步线程收集性能计数器产生的信息, 记录每一个数据块的近期被访问次数, 然后归入冷热队列. 数据迁移策略根据上述信息标记待迁移的数据块, 采用批处理的方式进行迁移. |
| 2023/01 | ADT, 主动的内存降级和升级策略. | [ADT: Aggressive Demotion and Promotion for Tiered Memory](https://dl.acm.org/doi/10.1109/LCA.2023.3236685) | 团队 | 主动降级方案将冷页面降级为慢速内存, 即使快速内存中的可用空间没有不足. 先前关于主动降级的工作通过减少应用程序在快速内存中的驻留集大小来降低对昂贵的快速内存容量的要求. 此外, 一些先前的工作减轻了当热数据需求激增时由于快速内存容量不足而导致的巨大性能下降. 然而, 通过进一步的主动降级来节省更大的快速内存容量, 还有进一步改进的空间, 可以充分收获上述主动降级的优势. 本文中提出了一种新的主动降级方案 ADT, 该方案对内存进行激进的降级和升级. ADT 使用应用程序和内存分配器分配内存的单元内的内存访问局部性, 从先前工作采用的页面扩展降级/升级单元, 使其降级更具侵略性. 通过扩展单元执行降级和升级, ADT 减少了 29% 的快速内存使用量, 而性能仅下降了 2.3%. 此外, 当系统的内存使用量大于快速内存容量时, 与默认的 Linux 内核相比, 它实现了 2.28× 的加速, 这优于最先进的分层内存管理方案. |
| 2023/03 | Memtis, 一种分层内存系统, 来决策页面放置和页面大小. 利用分配页面的访问分布, 以最佳方式将热数据集近似于快速层容量. 此外, Memtis 动态确定允许应用程序使用大页面的页面大小, 同时通过检测快速层内存的低效使用并在必要时将其拆分来避免其缺点. 我们的评估表明, Memtis 的性能比最先进的分层系统高出 169.0%, 比其最佳分层系统高出 33.6%. | [MEMTIS: Efficient Memory Tiering with Dynamic Page Classification and Page Size Determination, SOSP '23](https://dl.acm.org/doi/10.1145/3600006.3613167) | 团队 | 提出 Memtis, 一种利用 informed decision-making 机制来制定页大小和页放置策略的分层内存系统. Memtis 利用已分配页的访问分布, 以最优的方式逼近热数据集的fast tier capacity. Memtis 主要解决三个问题:<br>1. 如何使用基于硬件的内存访问采样, 以细粒度和轻量级的方式跟踪内存访问;<br>2. 如何根据整体内存访问频率分布, 动态准确地确定热页和冷页;<br>3. 如何动态确定页大小(大页与普通页), 以在不浪费 fast tier 内存的情况下降低转换成本. 具体来说:<br>1. Memtis 使用 Processor Event-Based Sampling (PEBS) 对内存访问进行采样. 由于 PEBS 样本包含精确的内存地址, 使得 Memtis 可以支持细粒度的访问跟踪. 在 Memtis 维护一个内核后台线程(ksampled)来处理采样的地址, Memtis 可以动态调整访存采样频率, 确保 CPU 开销在阈值(< 3%)以下.<br>2. Memtis 使用 page access counts 来维护所有已分配页面的热度分布(下图中的 page access histogram), 并且利用该数据做出最佳的 memory tiering 决策, 将最热的页放置在 fast tier 以最小化访问延迟.<br>3. Memtis 在 Linux 中默认使用 Transparent Huge Pages(THP) 来减少地址翻译的开销. 然而, 单一页大小并不适用于所有工作负载. 例如, 如果一个大页中只有一小部分子页被频繁访问(即, 大页的利用率很低时), 那么应该只将热点子页迁移到 fast tier. Memtis 通过 split benefit estimation 来检测这种情况. Memtis 使用 ksampled 维护的 emulated base page histogram 来估计仅使用普通页时的最大命中率, 并将估计的最大命中率与从 PEBS 的采样记录中获得的实际命中率进行比较, 以此来估计拆分大页的潜在收益(下图中绿色的部分), 如果潜在的好处很大, Memtis 会选择子页面中访存模式最不均匀的大页作为拆分候选页. 然后, 在后台对大页进行拆分, 并根据大页中维护的子页访问信息将每个分片后的子页放置到相应的内存层.<br> 参见 [Paper Read——Memtis: Efficient Memory Tiering with Dynamic Page Classification and Page Size Determi, SOSP'23](https://blog.csdn.net/bukuishiwoa/article/details/146371124), [论文阅读：Memtis: Efficient Memory Tiering with Dynamic Page Classification and Page Size Determination](https://blog.csdn.net/ghost_him/article/details/143182014), [论文阅读笔记整理(持续更新)](https://zhuanlan.zhihu.com/p/679168298), [SJTU-IPADS——SOSP 2023 科布伦茨小镇见闻(一)](https://zhuanlan.zhihu.com/p/663280265)<br>[slides](https://multics69.github.io/pages/pubs/memtis-lee-sosp23-slides.pdf), [cosmoss-jigu/memtis](https://github.com/cosmoss-jigu/memtis) |
| 2023/12 | FreqTier, 用于 CXL 的快速、准确、应用透明的基于频率的分层系统. | [Lightweight Frequency-Based Tiering for CXL Memory Systems——论文泛读](https://zhuanlan.zhihu.com/p/687833058) | 团队 | 针对利用本地内存和CXL内存的分层系统的两个关键挑战: (1) 如何在提升数据放置准确率的同时, (2)有效减少内存和运行时间开销.<br>本文提出 FreqTier, 用于 CXL 内存的基于频率的分层系统.<br>提出三个创新点:<br>(1). 采用了基于频率的分层, 使用高性能硬件计数器以页粒度跟踪内存访问.<br>(2). 使用计数布隆滤波器(CBF)(一种概率数据结构)跟踪页面访问频率.<br>(3). 采用动态方法, 根据应用程序内存访问行为调整分层操作的强度, 调整页面迁移和访问采样频率. 参见 [Lightweight Frequency-Based Tiering for CXL Memory Systems——论文泛读](https://zhuanlan.zhihu.com/p/687833058). |
| 2024/07 | Memstrata, 一种轻量级的多租户内存分配器, 提供 Intel Flat Memory Mode 的硬件分层内存 placement 方案, 采用页面着色来消除虚拟机间争用. 它通过使用联机速度减速估算器为具有对硬件分层敏感的访问模式的虚拟机分配更多本地 DRAM 来提高其性能. | [Managing Memory Tiers with CXL in Virtualized Environments, OSDI'24](https://www.usenix.org/conference/osdi24/presentation/zhong-yuhong) | 哥伦比亚大学 | 这篇论文应该是 Azure 在 CXL memory 上的一个早期探索和尝试, 云提供商希望利用 CXL memory 来增强内存带宽、容量并且减少碳排放/能耗, 其优势在于服务器上想要插更多的 DIMM 一方面很贵, 一方面受限于硬件, 而 CXL 可以挂在 PCIe, 并且可以自由使用 DDR4/DDR5/NVM, 不受限于相同的 DDR 代数. 进一步允许云厂商将要淘汰的 DDR4 作为 CXL memory 插上去, 从而减少成本. 不过 CXL memory 延迟比 Local Memory 高了很多(例如 Demystifying CXL Memory with Genuine CXL-Ready Systems and Devices @ MICRO'23 报告了 CXL memory 的 latency 比 remote NUMA 延迟可能会慢 >= 35%), 目前的很多分层内存系统就是为了解决这个问题, 目标是在利用 CXL memory 的情况下尽可能减少应用的 slowdown, 而大部分目前的方案都是 software-based 的, 将 CXL memory 暴露为一个 NUMA node, 然后通过追踪页面热度(page hotness tracking) 来将冷的页面移到 capacity tier(例如 CXL/NVM), 将热的页面移动到 performance tier(local memory).<br>这些传统 software-based 方案的问题是开销较大, 云提供商希望尽可能把每个服务器的 CPU 都卖出去, 而不是消耗很多CPU来做冷热内存识别<br>这篇文章提供的是硬件的分层内存 placement 方案, 叫做 Intel Flat Memory Mode, 作者说在 Intel Xeon 6 Processor 上会支持这个特性, 它基本是让 CPU 的 memory controller 以 Cache Line 为粒度来管理 CXL memory.<br>另外即使采用了硬件方案, 在某些 workload 下也会有较大的性能损失, 对于这种情况, 作者引入了软件辅助管理的方案, 这个软件系统叫做 Memstrata, 它主要解决两个问题: VM 之间的性能影响: 尽量不要让VM之间互相影响对方的内存性能. 参见 [Memstrata 论文笔记](https://zhuanlan.zhihu.com/p/708966564) 和 [Managing Memory Tiers with CXL in Virtualized Environments——论文阅读](https://blog.csdn.net/qq_36159989/article/details/140518348), 代码开源在 [cuhk-mass/memstrata](https://github.com/cuhk-mass/memstrata). 在原型硬件上的多虚拟机实验中, 能够识别性能异常值并将其从 30% 以上降低到 6% 以下, 从而在各种工作负载中提供一致的性能. |
| 2024/11 | Colloid, 基于"平衡各层访存时延"的原则迁移页面, 利用 Intel CHA(Caching and Home Agent) 硬件事件监听时延, 将页面转移到访问时延更低的层级中, 使得全系统平均时延和带宽到达接近最优值. | [Tiered Memory Management: Access Latency is the Key!, SOSP'24](https://dl.acm.org/doi/10.1145/3694715.3695968) | 团队 | 作者通过实验发现, 并不是把越多的热页面放进默认层级越好, 对某个层级的访存请求越多, 可能导致这个层级的访存时延劣化.<br>作者提出了内存管理机制 Colloid, 基于"平衡各层访存时延"的原则迁移页面, 将页面转移到访问时延更低的层级中, 最终平衡时默认和额外层级的时延基本相同, 从而动态地调节系统访问 default tier 内存的概率, 使得全系统平均时延和带宽到达接近最优值.<br>Colloid 利用了 Intel 处理器中的 CHA(Caching and Home Agent) 硬件计数器来统计读取时延. CHA 可以低开销地统计当前访存请求队列中通往默认和额外层级的时间占用和请求次数, 因此可以直接算出两个内存层级的时延, 以及默认层级访问概率 p. 如果默认层级的延迟低于备选层级, 则 Colloid 通过迁移热页面到默认层级增加 p(提升访问默认层级概率); 反之, 减少 p(降低访问默认层级概率). Colloid 通过多轮迁移中不断缩小搜寻范围, 尝试探查到使得两个层级访存时延相等的 p*，并使实际访问默认层级的概率逼近这一点. 如果系统的负载发生变化, 算法会重新探查 p*, 使得系统尽快恢复平衡.<br>[SOSP 2024 论文评述 Day 1 Session 2 Memory](https://zhuanlan.zhihu.com/p/5100600418), [Paper Read——Tiered Memory Management: Access Latency is the Key!](https://blog.csdn.net/bukuishiwoa/article/details/143723738), Code 开源在 [host-architecture/colloi](https://github.com/host-architecture/colloid). |
| 2024/11 | FlexMem, 自适应页面分析(结合多种页面分析方法)、动态降级(自适应页面降级)与温页范围(自适应温页范围), 提升热页识别与内存利用率. | [FlexMem: Adaptive Page Profiling and Migration for Tiered Memory, ATC'24](https://www.usenix.org/conference/atc24/presentation/xu-dong) | 团队 | FlexMem 包含三大核心设计:<br>1. 自适应页面分析(结合多种页面分析方法): 结合了基于 PEBS 的性能技术方法和基于页面提示故障的分析方法, 提高了页面热度识别的准确性和及时性. 使用统一的升级间隔, 为两种分析方法提供了平等的机会来贡献热页面. 当两种方法对页面识别结果不同时, 在两种方法对页面热度达成一致之前, 不会立即降级.<br>2. 自适应页面降级: 根据容纳热页面的需要, 动态决定降级的页面数量, 避免了因固定阈值导致的资源浪费或性能下降. 根据快速内存中冷页面数量、页面升级失败的频率和最近页面升级的有效性动态调整降级速率. 将页面热度的阈值与页面迁移进行耦合(MEMTIS 的这两者之间就没有进行耦合操作), 需要降级的页面的数量取决于在快速内存中的冷页面的数量以及需要升级的但是没有成功升级的页面数量.<br>3. 自适应温页范围: 根据页面升级为热页面的频率, 动态决定温页面范围, 提高了热页面识别的灵敏度, 防止不必要的降级, 除非可用的快速内存空间真的很稀缺. [Paper Read——FlexMem: Adaptive Page Profiling and Migration for Tiered Memory](https://blog.csdn.net/bukuishiwoa/article/details/145823645), [FlexMEM Adaptive Page Profiling and Miagration for Tiered Memory论文阅读](https://zhuanlan.zhihu.com/p/1913626863839979236) |
| 2025/01 | DSA-2LM, 适用于英特尔数据流加速器(DSA, Data Streaming Accelerator)的分层内存系统, 自适应地将 DSA 集成到页面迁移中. | [DSA-2LM: A CPU-Free Tiered Memory Architecture with Intel DSA, ATC'25](https://www.usenix.org/conference/atc25/presentation/liu-ruili) | 清华大学 | 分层内存对于管理异构内存设备(例如持久内存或 CXL 内存)至关重要. 现有工作在最佳数据放置和昂贵的数据移动之间做出了艰难的权衡. 随着英特尔数据流加速器(DSA, Data Streaming Accelerator)的出现, 这是一种无需 CPU 的硬件, 可在内存区域之间移动数据, 数据移动速度比单个 CPU 内核快 4×. 然而, Linux 内核中精细的内存移动粒度破坏了潜在的性能改进. 为此, 作者开发 DSA-2LM, 这是一种新的分层内存系统, 可自适应地将 DSA 集成到页面迁移中. 所提出的框架将快速内存迁移工作流程和适应性强的并发数据路径与经过良好调整的 DSA 配置集成在一起. 实验结果表明, 与 MEMTIS、TPP 和 NOMAD 三种具有代表性的分层存储器作品相比, DSA-2LM 在实际应用下可以实现 20%、30%和 16%的性能提升. |
| 2025/03 | PET, 将降级和升级的单位从先前工作采用的作系统页面扩展到 PET 块(P-block) 的主动降级方案. | [PET: Proactive Demotion for Efficient Tiered Memory Management, EuroSys'25](https://dl.acm.org/doi/10.1145/3689031.3717471) | 团队 | 主动降级是有效使用分层内存的方法之一, 即使快速内存有足够的可用空间, 它也会将冷数据降级为慢速内存. 以前的工作已经利用主动降级来降低主内存的高成本, 方法是减少应用程序在快速内存中的驻留集大小, 此外, 主动降级有助于缓解在热数据需求激增时因快速内存短缺而导致的严重性能下降. 尽管如此, 作者观察到, 利用应用程序分配单元中的内存访问局部性可以实现更大的快速内存节省, 同时降低系统开销.<br>作者提出了一种新的主动降级方案 PET, 它执行主动降级以实现高效的分层内存管理. PET 建议将降级和升级的单位从先前工作采用的作系统页面扩展到 PET 块(P-block), 它反映了应用程序分配内存的单位. 作者提供了仔细选择降级目标 P 块并在访问模式发生变化时迅速提升降级 P 块的机制. Linux 内核 v6.1.44 上的 PET 原型减少了 39.8%(高达 80.4%)的快速内存使用量, 平均仅降低 1.7% 的评估工作负载性能. 此外, 当系统的内存使用量大于快速内存容量时, 与默认的 Linux 内核相比, 它可以减轻 31% 的性能下降, 这优于最先进的分层内存管理方案. |
| 2025/06 | ArtMem, 强化学习(RL)驱动的动态分层内存系统. | [ArtMem: Adaptive Migration in Reinforcement Learning-Enabled Tiered Memory, ISCA'25](https://dl.acm.org/doi/abs/10.1145/3695053.3731001) | 团队 | 随着新兴应用的内存需求不断增加, 分层内存已成为降低数据中心硬件成本的可行解决方案. 鉴于分层内存系统中容量层的性能较低, 优化内存管理对于提高整体系统性能至关重要. 本文确定了现有分层内存解决方案中的三个关键限制. 首先, 现有解决方案在不同工作负载中的性能通常不同, 导致某些工作负载的性能不佳. 其次, 他们往往无法调整迁移策略以应对低快速内存层访问率, 导致数据放置无效. 第三, 他们经常错过根据工作负载模式动态调整内存迁移范围的机会, 导致不必要的页面迁移和分层内存潜力利用率不足. 本文提出了 ArtMem, 这是一个强化学习(RL)驱动的框架, 可以动态管理分层内存系统并适应工作负载演变以解决这些限制. ArtMem 可以更好地放置内存页面, 增强系统性能, 同时减少不必要的迁移. 实验评估表明, ArtMem 的性能优于最先进的分层系统, 在各种工作负载上实现了 35% - 172% 的性能提升. |
| 2025/07 | HybridTier, 内存的自适应轻量级分层系统.<br>1. 通过跟踪长期数据访问频率和短期访问动量, 以准确捕捉和适应不断变化的热度分布.<br>2. 通过跟踪数据访问概率来减少元数据内存开销, 通过权衡少量跟踪不准确性来获得更高的内存效率. | [HybridTier: an Adaptive and Lightweight CXL-Memory Tiering System](https://arxiv.org/abs/2312.04789v2) | 团队 | 现代工作负载对越来越大的内存容量的要求越来越大. 基于 Compute Express Link(CXL) 的内存分层已成为解决此问题的有前途的解决方案, 通过利用传统 DRAM 和慢层 CXL 内存设备. 作者分析以前的分层系统, 并观察到高性能内存分层面临的两个挑战: 适应倾斜但动态变化的数据热度分布, 同时最大限度地减少分层引起的内存和缓存开销.<br>为了应对这些挑战, 本文提出了 HybridTier, 这是一种用于 CXL 内存的自适应轻量级分层系统. HybridTier 跟踪长期数据访问频率和短期访问动量, 以准确捕捉和适应不断变化的热度分布. HybridTier 通过跟踪数据访问概率来减少元数据内存开销, 通过权衡少量跟踪不准确性来获得更高的内存效率, 这对应用程序性能的影响可以忽略不计. 为了减少缓存开销, HybridTier 使用轻量级数据结构来优化数据局部性, 以跟踪数据热度. 评估表明, HybridTier 的性能优于以前的系统 91%, 高达产生的内存开销少 2.0-7.8x, 缓存未命中也少 1.7-3.5x. |
...
| 2023/08/02 | SeongJae Park <sj@kernel.org> | [Extedn DAMOS filters for address ranges and DAMON monitoring targets](https://patchwork.kernel.org/project/linux-mm/cover/20230802214312.110532-1-sj@kernel.org/) | 772342 | v1 ☐☑ | [LORE v1,0/13](https://lore.kernel.org/r/20230802214312.110532-1-sj@kernel.org) |
...
此外 [uksmd](https://codeberg.org/pf-kernel/uksmd) 提供了一个 Userspace KSM helper daemon, [CachyOS](https://github.com/CachyOS/uksmd) 将其用在了自己的发行版中. 守护进程会定期检查用户空间任务列表, 并通知 ksmd kthread 设置相关进程 mm_struct 的 MM_VM_MERGE_ANY 标记, 以自动合并具有相同内容的内存页. 仅处理长期任务. 该机制依赖于 v6.4 [mm: process/cgroup ksm support](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/log/?id=07115fcc15b4aa5c268fb80b82ad15868a82a285) 以及 [pf-kernel/linux](https://codeberg.org/pf-kernel/linux) 内核提供的 `process_ksm_{enable,disable,status}()` 系统调用.
...
| 2022/10/08 | xu.xin.sc@gmail.com <xu.xin.sc@gmail.com> | [ksm: support tracking KSM-placed zero-pages](https://lore.kernel.org/all/20221008070156.308465-1-xu.xin16@zte.com.cn) | 在使能 use_zero_pages 之前, ksm 的 pages_sharing 基本是准确的. 但是当启用 use_zero_pages 时, 所有与内核零页合并的空页都不会计算在 pages_sharing 或 pages_shared 中. 这是因为这些空页面被合并为零页面, KSM 不再管理这些页面, 这至少会导致两个问题: 1. MADV_UNMERGEABLE 和其他触发取消共享的方法将不会取消 KSM 放置的共享零页(这至少是违反 MADV_UNMERGEABLE 文档的), 参见[链接](https://lore.kernel.org/lkml/4a3daba6-18f9-d252-697c-197f65578c44@redhat.com). 2. 当启用 use_zero_pages 时, 我们无法知道有多少页是 KSM 放置的零页, 这导致 KSM 对所有实际合并的页面不透明. 通过这个补丁集我们可以精确地取消共享零页 (ksm - 放置) 并计算 ksm 零页. | v1 ☐☑✓ | [2022/10/08 LORE v1,0/5](https://lore.kernel.org/all/20221008070156.308465-1-xu.xin16@zte.com.cn)<br>*-*-*-*-*-*-*-* <br>[2022/10/11 LORE v3,0/5](https://lore.kernel.org/all/20221011022006.322158-1-xu.xin16@zte.com.cn)<br>*-*-*-*-*-*-*-* <br>[2022/12/26 LORE v4,0/6](https://lore.kernel.org/r/202212260959492929897@zte.com.cn)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/6](https://lore.kernel.org/r/202212300911327101708@zte.com.cn)<br>*-*-*-*-*-*-*-* <br>[LORE v7,0/6](https://lore.kernel.org/r/202304131346489021903@zte.com.cn)<br>*-*-*-*-*-*-*-* <br>[2023/05/22 LORE v8,0/6](https://lore.kernel.org/r/202305221842587200002@zte.com.cn)<br>*-*-*-*-*-*-*-* <br>[LORE v9,0/5](https://lore.kernel.org/r/202305241351365661923@zte.com.cn)<br>*-*-*-*-*-*-*-* <br>[LORE v10,0/5](https://lore.kernel.org/r/202305251341580149313@zte.com.cn) |
| 2023/04/06 | Stefan Roesch <shr@devkernel.io> | [mm: process/cgroup ksm support](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/log/?id=07115fcc15b4aa5c268fb80b82ad15868a82a285) | 到目前为止, KSM 只能通过为内存区域调用 madvise 来启用. 这组补丁允许在进程 / cgroup 级别启用/禁用 KSM. 参见 [Process-level kernel samepage merging control](https://lwn.net/Articles/928510) | v1 ☐☑ v6.4-rc1 | [LORE v1,0/20](https://lore.kernel.org/r/20230123173748.1734238-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/19](https://lore.kernel.org/r/20230210215023.2740545-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/3](https://lore.kernel.org/r/20230224044000.3084046-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v4,0/3](https://lore.kernel.org/all/20230310182851.2579138-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/3](http://lore.kernel.org/all/20230406165339.1017597-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/3](https://lore.kernel.org/r/20230406165339.1017597-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v7,0/3](https://lore.kernel.org/r/20230413233115.1878303-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v8,0/3](https://lore.kernel.org/r/20230415225913.3206647-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v9,0/3](https://lore.kernel.org/all/20230418051342.1919757-1-shr@devkernel.io) |
| 2023/02/10 | Stefan Roesch <shr@devkernel.io> | [[v1] mm: add tracepoints to ksm](https://patchwork.kernel.org/project/linux-mm/patch/20230210214645.2720847-1-shr@devkernel.io/) | 720845 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230210214645.2720847-1-shr@devkernel.io)<br>*-*-*-*-*-*-*-* <br>[LORE v2,0/1](https://lore.kernel.org/r/20230313204606.3581988-1-shr@devkernel.io) |
...
| 2024/03/29 | Hui Zhu <teawater@gmail.com> | [mm: Introduce uKSM for user-controlled KSM](https://lore.kernel.org/all/20240329104035.62942-1-teawater@antgroup.com) | 作者提供了一个使用 uKSM 接口跟踪和合并指定任务页面的示例应用程序, 可在 GitHub 获取 [teawater/uksmd](https://github.com/teawater/uksmd). | v1 ☐☑✓ | [LORE](https://lore.kernel.org/all/20240329104035.62942-1-teawater@antgroup.com) |
...
| 2022/12/19 | Tong Tiangen <tongtiangen@huawei.com> | [arm64: add machine check safe support](https://patchwork.kernel.org/project/linux-mm/cover/20221219120008.3818828-1-tongtiangen@huawei.com/) | 705578 | v8 ☐☑ | [LORE v8,0/4](https://lore.kernel.org/r/20221219120008.3818828-1-tongtiangen@huawei.com)<br>*-*-*-*-*-*-*-* <br>[LORE v9,0/5](https://lore.kernel.org/r/20230508014436.198717-1-tongtiangen@huawei.com) |
| 2023/04/11 | Liu Shixin <liushixin2@huawei.com> | [[-next] mm: hwpoison: support recovery from HugePage copy-on-write faults](https://patchwork.kernel.org/project/linux-mm/patch/20230411092741.780679-1-liushixin2@huawei.com/) | 738678 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230411092741.780679-1-liushixin2@huawei.com) |
| 2023/04/13 | Kefeng Wang <wangkefeng.wang@huawei.com> | [mm: hwpoison: coredump: support recovery from dump_user_range()](https://patchwork.kernel.org/project/linux-mm/patch/20230413041336.26874-1-wangkefeng.wang@huawei.com/) | 739345 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/20230413041336.26874-1-wangkefeng.wang@huawei.com) |
...
| 时间 | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:---:|:----:|:---:|:----:|:---------:|:----:|
| 2025/09/12 | Jinchao Wang <wangjinchao600@gmail.com> | [mm/ksw: Introduce real-time KStackWatch debugging tool](https://lore.kernel.org/all/20250912101145.465708-1-wangjinchao600@gmail.com) | 引入一个名为 **KStackWatch** 的轻量级内核调试工具, 用于**实时检测内核栈破坏**问题. 该工具通过结合**硬件断点( HWBP) 、kprobe 与 fprobe**, 在栈金丝雀或局部变量被破坏的瞬间捕捉异常, 从而帮助开发者准确定位问题源头, 尤其适用于那些破坏发生与崩溃表现不一致、难以复现的场景.<br>关键特性包括:<br>1. 实时检测栈破坏, 开销小, 不影响问题复现; <br>2. 通过 `/proc/kstackwatch` 简化配置; <br>3. 支持递归深度过滤; <br>4. 支持CPU热插拔. | v4 ☐☑✓ | [2025/09/12, LORE v4, 00/21](https://lore.kernel.org/all/20250912101145.465708-1-wangjinchao600@gmail.com) |
...
| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2021/09/22 | John Hubbard <jhubbard@nvidia.com> | [Mitosis: Transparently Self-Replicating Page-Tables for Large-Memory Machines](https://research.vmware.com/files/attachments/0/0/0/0/1/0/3/aspl0359a-achermanna.pdf) | NUMA 迁移的时候考虑迁移 page-table. 在 NUMA 系统下, 内存的增长速度超过了 TLB 容量的增长速度, 从而可能引入严重的性能问题, 而 NUMA 已有的研究都集中在的内存中数据的分配和迁移策略上, 但没有关于如何在各 NUMA 之间放置页表的问题的研究. Mitosis 实现了一种程序无感的页表复制和迁移算法, 通过透明地跨 NUMA 复制和迁移页表来减轻 NUMA 对页表遍历的影响, 这降低了在 Page Walk 执行页表遍历时访问远程 NUMA 节点的频率. | v1 ☐ | [GitHub](https://github.com/mitosis-project/mitosis-linux) |
...
| 时间  | 作者 | 特性 | 描述 | 是否合入主线 | 链接 |
|:----:|:----:|:---:|:----:|:---------:|:----:|
| 2020/05/07 | Anthony Yznaga <anthony.yznaga@oracle.com> | [PKRAM: Preserved-over-Kexec RAM](https://lore.kernel.org/patchwork/patch/856356) | NA | v11 ☑ 4.15-rc2 | [PatchWork RFC,00/43](https://lore.kernel.org/patchwork/patch/1237362)<br>*-*-*-*-*-*-*-* <br>[LORE v3,0/21](https://lore.kernel.org/r/1682554137-13938-1-git-send-email-anthony.yznaga@oracle.com) |


## 14.11 unaccepted memory
-------

[Seeking an acceptable unaccepted memory policy](https://lwn.net/Articles/928328)
...
| 2023/03/30 | Kirill A. Shutemov <kirill.shutemov@linux.intel.com> | [mm, x86/cc: Implement support for unaccepted memory](https://patchwork.kernel.org/project/linux-mm/cover/20221207014933.8435-1-kirill.shutemov@linux.intel.com/) | Intel TDX 至此 unaccepted memory. | v1 ☐☑ | [LORE v1,0/14](https://lore.kernel.org/r/20221207014933.8435-1-kirill.shutemov@linux.intel.com)<br>*-*-*-*-*-*-*-* <br>[LORE v9,0/14](https://lore.kernel.org/all/20230330114956.20342-1-kirill.shutemov@linux.intel.com)<br>*-*-*-*-*-*-*-* <br>[LORE v1,0/11](https://lore.kernel.org/r/20230507234618.18067-1-kirill.shutemov@linux.intel.com)<br>*-*-*-*-*-*-*-* <br>[LORE v1,0/9](https://lore.kernel.org/r/20230513220418.19357-1-kirill.shutemov@linux.intel.com)<br>*-*-*-*-*-*-*-* <br>[LORE v1,0/9](https://lore.kernel.org/r/20230518231434.26080-1-kirill.shutemov@linux.intel.com) |
| 2023/04/04 | Tom Lendacky <thomas.lendacky@amd.com> | [Provide SEV-SNP support for unaccepted memory](https://lore.kernel.org/all/cover.1680628986.git.thomas.lendacky@amd.com) | AMD SEV-SNP 支持 unaccepted memory. | v9 ☐☑✓ | [LORE v7,0/6](https://lore.kernel.org/all/cover.1680628986.git.thomas.lendacky@amd.com) |
...
| 2023/04/03 | Carlos Maiolino <cem@kernel.org> | [shmem: Add user and group quota support for tmpfs](https://patchwork.kernel.org/project/linux-mm/cover/20230403084759.884681-1-cem@kernel.org/) | 736280 | v1 ☐☑ | [LORE v1,0/6](https://lore.kernel.org/r/20230403084759.884681-1-cem@kernel.org)<br>*-*-*-*-*-*-*-* <br>[LORE v5,0/6](https://lore.kernel.org/r/20230717115212.208651-1-cem@kernel.org)<br>*-*-*-*-*-*-*-* <br>[LORE v6,0/7](https://lore.kernel.org/r/20230725144510.253763-1-cem@kernel.org) |
| 2023/04/06 | Chuck Lever <cel@kernel.org> | [[RFC] shmem: stable directory cookies](https://patchwork.kernel.org/project/linux-mm/patch/168080987776.946167.3501480439542616457.stgit@manet.1015granger.net) | 737771 | v1 ☐☑ | [LORE v1,0/1](https://lore.kernel.org/r/168080987776.946167.3501480439542616457.stgit@manet.1015granger.net) |
